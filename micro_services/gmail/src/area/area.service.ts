...
export class AreaService {
    ...
    async getAvailableArea(): Promise<AreaResponse> {
        ...
        actions.forEach(action => {
            this.logger.log(`Action: ${action.name}`);
            this.logger.log(`ConfigSchema type: ${typeof action.configSchema}`);
            this.logger.log(`ConfigSchema value: ${JSON.stringify(action.configSchema, null, 2)}`);
        });
