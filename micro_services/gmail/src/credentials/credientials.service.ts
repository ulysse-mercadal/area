...
export class CredentialsService {
    ...
    constructor(
        ...
        this.serviceId = parseInt(
            this.configService.get<string>('SERVICE_ID') || '6',
        );
    }
    ...
    async updateLastHistoryId(
        ...
        try {
            ...
            this.logger.debug(
            `Updated lastHistoryId for user ${userId} to ${historyId}`,
            );
        } catch (error) {
            this.logger.error(
            `Failed to update lastHistoryId for user ${userId}: ${error.message}`,
            );
            throw error;
        }
    }
