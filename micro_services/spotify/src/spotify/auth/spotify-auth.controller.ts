...
export class SpotifyAuthController {
    ...
    async initiateAuth(@Body('userId') userId: number) {
        try {
            const authUrl = this.spotifyAuthService.getAuthUrl(userId.toString());
            return {
                success: true,
                authUrl,
            ...
            this.logger.error('Failed to initiate Spotify auth:', error.message);
            return {
                success: false,
                error: error.message,
            };
    ...
    async handleCallback(
        ...
        try {
            if (!code) {
                return res.redirect(`${frontendUrl}/settings/services?spotify=error&reason=no_code`);
            }
            const userId = parseInt(state, 10);
            if (isNaN(userId)) {
                return res.redirect(`${frontendUrl}/settings/services?spotify=error&reason=invalid_user`);
            }
            const tokenResponse = await this.spotifyAuthService.exchangeCodeForToken(code);
            ...
            this.logger.log(`Successfully saved Spotify credentials for user ${userId}`);
            return res.redirect(`${frontendUrl}/settings/services?spotify=success`);
        } catch (error) {
            this.logger.error('Spotify callback error:', error.message, error.stack);
            return res.redirect(`${frontendUrl}/settings/services?spotify=error&reason=callback_failed`);
        }
    }
    ...
    async refreshToken(@Body('userId') userId: number) {
        try {
            ...
            const tokenData = await this.spotifyAuthService.refreshAccessToken(
    ...
    async disconnect(@Body('userId') userId: number) {
        try {
            ...
            return {
                success: true,
                message: 'User disconnected from Spotify',
            };
        } catch (error) {
