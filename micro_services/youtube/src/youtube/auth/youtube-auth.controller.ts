...
export class youtubeAuthController {
    ...
    async initiateAuth(@Body('userId') userId: number) {
        try {
            const authUrl = this.youtubeAuthService.getAuthUrl(userId.toString());
            return {
                success: true,
                authUrl,
            ...
            this.logger.error('Failed to initiate youtube auth:', error.message);
            return {
                success: false,
                error: error.message,
            };
    ...
    async handleCallback(
        ...
        try {
            if (!code) {
                return res.redirect(`${frontendUrl}/settings/services?youtube=error&reason=no_code`);
            }
            const userId = parseInt(state, 10);
            if (isNaN(userId)) {
                return res.redirect(`${frontendUrl}/settings/services?youtube=error&reason=invalid_user`);
            }
            const tokenResponse = await this.youtubeAuthService.exchangeCodeForToken(code);
            ...
            this.logger.log(`Successfully saved youtube credentials for user ${userId}`);
            return res.redirect(`${frontendUrl}/settings/services?youtube=success`);
        } catch (error) {
            this.logger.error('youtube callback error:', error.message, error.stack);
            return res.redirect(`${frontendUrl}/settings/services?youtube=error&reason=callback_failed`);
        }
    }
    ...
    async disconnect(@Body('userId') userId: number) {
        try {
            ...
            return {
                success: true,
                message: 'User disconnected from youtube',
            };
        } catch (error) {
