...
@Controller('google-sheets/auth')
export class GoogleSheetsAuthController {
    private readonly logger = new Logger(GoogleSheetsAuthController.name);
    ...
    async initiateAuth(@Body('userId') userId: number) {
        try {
            const authUrl = this.googleSheetsAuthService.getAuthUrl(userId.toString());
            return {
                success: true,
                authUrl,
    ...
    async handleCallback(
        ...
        try {
            if (!code) {
                return res.redirect(`${frontendUrl}/settings/services?google_sheets=error&reason=no_code`);
            }
            const userId = parseInt(state, 10);
            if (isNaN(userId)) {
                return res.redirect(`${frontendUrl}/settings/services?google_sheets=error&reason=invalid_user`);
            }
            const tokenResponse = await this.googleSheetsAuthService.exchangeCodeForToken(code);
            ...
            this.logger.log(`Successfully saved Google Sheets credentials for user ${userId}`);
            return res.redirect(`${frontendUrl}/settings/services?google_sheets=success`);
        } catch (error) {
            this.logger.error('Google Sheets callback error:', error.message, error.stack);
            return res.redirect(`${frontendUrl}/settings/services?google_sheets=error&reason=callback_failed`);
        }
    }
    ...
    async refreshToken(@Body('userId') userId: number) {
        try {
            ...
            const tokenData = await this.googleSheetsAuthService.refreshAccessToken(
    ...
    @Get('test/list-spreadsheets/:userId')
    async testListSpreadsheets(@Param('userId') userId: string) {
        try {
            const userIdNum = parseInt(userId, 10);
            const credentials = await this.getCredentialsFromMainApi(userIdNum);
            ...
            const result = await this.googleSheetsAuthService.listSpreadsheets(
                credentials.token,
                100
            );
            return {
